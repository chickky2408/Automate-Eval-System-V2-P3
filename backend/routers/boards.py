...
def _board_to_fe(board: BoardInfo) -> dict:
    ...
    return {
        ...
        "temp": board.status.cpu_temp if not is_offline else None,
        "cpuLoad": board.status.cpu_load if not is_offline else None,
        "ramUsage": board.status.ram_usage if not is_offline else None,
